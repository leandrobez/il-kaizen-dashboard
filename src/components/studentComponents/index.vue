            ...
            <tbody v-if="hasStudents">
                <tr v-for="(list,index) in students" :key="index" :class="list.ativo ? 'il-active' : 'il-inactive'">
                    ...
                    <td>
                        ...
                        <i class="mdi mdi-12px mdi-script il-color--yellow" title="Receber pagamento do cliente com cheque" @click.prevent="makePayment(list._id,'check')"></i> |
                        <i class="mdi mdi-12px mdi-square-inc-cash il-color--dark" title="Receber pagamento do cliente com dinheiro" @click.prevent="makePayment(list._id,'money')"></i> |
